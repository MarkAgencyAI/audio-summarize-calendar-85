...
export function PdfUploader() {
  ...
  // Interfaz de usuario
  return <div className="space-y-4 rounded-xl p-4 md:p-6 shadow-lg mb-8 bg-custom-text">
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Input id="pdf-file" type="file" accept="application/pdf" onChange={handleFileChange} className="cursor-pointer bg-white/10 border-white/20 text-white dark:bg-custom-secondary/30 dark:border-custom-secondary/60" ref={fileInputRef} />
        </div>
        
        {file && <>
            <div className="space-y-2">
              <Label htmlFor="lesson-name" className="text-white">Nombre de la lección</Label>
              <Input id="lesson-name" value={lessonName} onChange={e => setLessonName(e.target.value)} placeholder="Nombre de la lección" className="bg-white/10 border-white/20 text-white placeholder:text-white/70 dark:bg-custom-secondary/30 dark:border-custom-secondary/60" />
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="folder" className="text-white">Carpeta</Label>
              <select id="folder" className="w-full h-10 px-3 py-2 bg-white/10 border border-white/20 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-white/50 dark:bg-custom-secondary/30 dark:border-custom-secondary/60 dark:focus:ring-custom-accent/50" value={selectedFolder} onChange={e => setSelectedFolder(e.target.value)}>
                {folders.map((folder: Folder) => <option key={folder.id} value={folder.id}>
                    {folder.name}
                  </option>)}
              </select>
            </div>
            
            <Button onClick={handleProcessPdf} disabled={loading} className="w-full bg-custom-secondary text-white hover:bg-custom-secondary/90 dark:bg-custom-accent dark:hover:bg-custom-accent/90">
              {loading ? <>
      ...
      {showAnalysis && <div className="mt-6 space-y-4">
          <Card className="bg-white/10 border-white/20 text-white dark:bg-custom-secondary/20 dark:border-custom-secondary/40">
            ...
            <CardContent>
              {keyPoints && keyPoints.length > 0 && <div className="mb-4">
